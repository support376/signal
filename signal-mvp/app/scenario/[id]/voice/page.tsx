...
/* ── 브라우저에서 음성 특성 추출 ── */
async function extractVoiceFeatures(blob: Blob, durationSec: number) {
  try {
    const audioCtx = new OfflineAudioContext(1, 44100 * Math.ceil(durationSec + 1), 44100);
    const arrayBuf = await blob.arrayBuffer();
    const audioBuffer = await audioCtx.decodeAudioData(arrayBuf);
    const channelData = audioBuffer.getChannelData(0);

    // Pitch 추정 (자기상관 기반 — 단순화된 구현)
    ...
    for (let start = 0; start + frameSize < channelData.length; start += hopSize) {
      ...
      const pitch = estimatePitch(frame, audioBuffer.sampleRate);
      if (pitch > 50 && pitch < 600) pitches.push(pitch);
    }
    const pitchMean = pitches.length > 0 ? pitches.reduce((a, b) => a + b, 0) / pitches.length : null;
    const pitchStd = pitches.length > 1
      ? Math.sqrt(pitches.reduce((acc, p) => acc + (p - pitchMean!) ** 2, 0) / (pitches.length - 1))
      : null;

    // RMS Energy
    let energySum = 0;
    for (let i = 0; i < channelData.length; i++) energySum += channelData[i] ** 2;
    const energyMean = Math.sqrt(energySum / channelData.length);

    // Spectral centroid (간소화)
    const fftSize = 2048;
    const fftFrame = channelData.slice(0, fftSize);
    const spectralCentroid = computeSpectralCentroid(fftFrame, audioBuffer.sampleRate);

    return {
      pitch_mean: pitchMean ? Math.round(pitchMean * 10) / 10 : null,
      pitch_std: pitchStd ? Math.round(pitchStd * 10) / 10 : null,
      energy_mean: Math.round(energyMean * 10000) / 10000,
      spectral_centroid: spectralCentroid ? Math.round(spectralCentroid) : null,
      mfcc_summary: null,  // MFCC는 서버에서 더 정밀하게 추출
      duration_sec: Math.round(durationSec * 10) / 10,
      sample_rate: audioBuffer.sampleRate,
    };
  } catch (e) {
    console.warn('Voice feature extraction failed:', e);
    return {
      pitch_mean: null, pitch_std: null,
      energy_mean: null, spectral_centroid: null,
      mfcc_summary: null,
      duration_sec: Math.round(durationSec * 10) / 10,
      sample_rate: 44100,
    };
...
function estimatePitch(frame: Float32Array, sampleRate: number): number {
  const minLag = Math.floor(sampleRate / 600);  // 600Hz 상한
  const maxLag = Math.floor(sampleRate / 50);   // 50Hz 하한
  let bestCorr = -1;
  let bestLag = minLag;
...
/** 스펙트럴 센트로이드 추정 */
function computeSpectralCentroid(frame: Float32Array, sampleRate: number): number | null {
  // 간소화된 DFT magnitude
...
function VoiceScenarioInner() {
  ...
  async function loadState(uid: string) {
    ...
    setError('');
    try {
      const r = await fetch('/api/scenario/voice-state', {
        method: 'POST',
  ...
  async function startScenario() {
    ...
    setShowIntro(false);
    try {
      const r = await fetch('/api/scenario/voice-turn', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  /* ── 음성 전송 — FormData로 audio blob + voice features 전송 ── */
  async function sendVoiceResponse() {
    ...
    try {
      // 브라우저에서 voice features 추출
      const voiceFeatures = await extractVoiceFeatures(audioBlob, currentDuration);

      // FormData로 전송
      const formData = new FormData();
      formData.append('userId', userId);
      formData.append('scenarioId', scenarioId);
      formData.append('userMessage', '🎙 음성 응답');
      formData.append('audio', audioBlob, `voice_${scenarioId}_${Date.now()}.webm`);
      formData.append('audioDuration', String(currentDuration));
          ...
          <div className="w-full bg-card border border-line rounded-2xl p-8 space-y-6">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] font-mono text-dim px-2 py-0.5 border border-line rounded">VOICE MODE</span>
              </div>
            ...
            <div className="border-t border-line pt-6 text-xs text-dim leading-relaxed">
              이번에는 <span className="text-fg font-medium">목소리</span>로 대답합니다.
              녹음 버튼을 누르고 말하면 돼. 떠오르는 대로 자연스럽게.
              <br /><br />
              <span className="text-faint">음성 특성 (피치, 에너지, 톤)이 함께 기록됩니다.</span>
            </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 flex flex-col min-h-screen">
      <header className="mb-4">
        <button onClick={() => router.push('/scenario')} className="text-xs text-dim hover:text-fg">
          ← Signal
        </button>
        <div className="flex items-center gap-2 mt-2">
          <h1 className="text-lg font-bold text-fg">{SCENARIO_LABELS[scenarioId]}</h1>
          <span className="text-[10px] font-mono text-dim px-1.5 py-0.5 border border-line rounded">VOICE</span>
        </div>
                ...
                <div className="bg-accent/15 border border-accent/25 rounded-2xl rounded-br-sm px-4 py-3">
                  {t.user_audio_url ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-dim">음성 응답</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="max-w-2xl mx-auto">
            {recState === 'idle' && (
              <button
                ...
                className="w-full flex items-center justify-center gap-3 py-4 border border-line rounded-2xl hover:bg-card transition group"
              >
                ...
                <span className="text-sm text-dim">탭하여 녹음 시작</span>
              </button>
            )}
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="px-5 py-2.5 bg-card border border-line rounded-xl text-sm text-fg hover:bg-card-hover transition"
                  >
                    ■ 녹음 완료
                  </button>
                </div>
            ...
            {recState === 'recorded' && audioUrl && (
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-3 border border-line rounded-xl">
                  ...
                  <span className="text-xs text-faint font-mono">{formatDuration(recordingDuration)}</span>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={resetRecording}
                    className="flex-1 py-3 border border-line rounded-xl text-sm text-dim hover:text-fg transition"
                  >
                    다시 녹음
                  </button>
                  <button
                    onClick={sendVoiceResponse}
                    className="flex-1 py-3 bg-fg text-bg rounded-xl text-sm font-semibold hover:opacity-80 transition"
                  >
                    전송 ↑
                  </button>
                </div>
