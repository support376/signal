      ...
      {narrative && !loading && (
        <div className={`mt-8 ${revealed ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="bg-card border border-line rounded-2xl p-8 text-center mb-6">
            ...
            <div className="text-7xl font-bold text-fg my-4">
              {scoreAnim}<span className="text-3xl">%</span>
            </div>

            {reliability && reliability !== '높음' && (
              <p className="text-xs text-warn mb-3">신뢰도: {reliability} (일부만 본 결과)</p>
            )}
            ...
            {tags.length > 0 && (
              <div className="flex flex-wrap justify-center gap-2">
                {tags.map((t) => (
                  <span key={t} className="px-3 py-1 bg-card border border-line rounded-full text-xs text-dim">
                    {t}
                  </span>
                ))}
              </div>
          ...
          {coreText && (
            <SectionCard emoji="⚡" title="핵심 역학" summary={coreText.slice(0, 60) + '...'} defaultOpen>
              <article className="max-w-none mt-4" dangerouslySetInnerHTML={{ __html: renderMarkdown(coreText) }} />
            </SectionCard>
          )}
            ...
            {sections.map((sec, i) => {
              ...
              return (
                <SectionCard
                  ...
                  defaultOpen={i < 2}
                >
                  <article className="max-w-none mt-4" dangerouslySetInnerHTML={{ __html: renderMarkdown(sec.content) }} />
                </SectionCard>
              );
            })}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center pt-6 border-t border-line">
            <button
              ...
              className="px-6 py-3 border border-line text-fg rounded-xl text-sm hover:bg-card"
            >
              이 결과 스크린샷 → 친구한테 보내봐
            </button>
            <button
